defmodule AlarmClock.Led do
  use GenServer

  alias AlarmClock.Button
  ...
  def handle_call(:on, {pid, _tag}, monitors) do
    if monitors == %{} do
      PubSub.broadcast!(AlarmClock.PubSub, @topic, {__MODULE__, :on})
    end
  ...
  def handle_call(:off, {pid, _tag}, monitors) do
    ...
    if monitor_ref do
      ...
      if remaining_monitors == %{} do
        PubSub.broadcast!(AlarmClock.PubSub, @topic, {__MODULE__, :off})
      end
    end
